...
class ModelBsmMC(ModelABC):
    ...
    def price(self, strike, spot, texp=1.0, cp=1):
        ...
        '''
        vol_path,Z = self.sigma_path(texp)
        ...
        W = np.random.standard_normal((n_dt, self.n_path))
        W= self.rho*Z +np.sqrt(1-self.rho**2)*W
        W_t = np.cumsum(W * np.sqrt(dt), axis=0)

        S_0 = spot * np.ones(self.n_path)
        S_T = np.ones((n_dt,self.n_path))
        S_T = np.insert(S_T, 0, S_0, axis=0)
        S_T= S_0*np.exp(np.cumsum(vol_path[:-1,:]*np.sqrt(dt)*W-0.5*(vol_path[:-1,:]**2)*dt,axis=0))
        '''S_T= S_0*np.exp(vol_path[1,:]*W_t - 0.5*(vol_path[1,:]**2)*tobs[:,None])'''
        '''for t in range(n_dt):
            S_T[t+1] = S_T[t]*np.exp(vol_path[t]*np.sqrt(dt)*W[t] - 0.5*(vol_path[t]**2)*dt)'''

        df = np.exp(-self.intr * texp)
        p = df * np.mean(np.fmax(cp*(S_T[-1,:] - strike[:, None]), 0.0), axis=1)
        return p
...
class ModelBsmCondMC(ModelBsmMC):
    ...
    def price(self, strike, spot, texp=1.0, cp=1):
        ...
        spot_equiv = spot * np.exp((self.rho/self.vov)*(sigma_t-self.sigma)-0.5*(self.rho**2)*(self.sigma**2)*texp*I_t)
