...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        #vol_bsm=vol/forward
        #bsm_cov_m=vol_bsm*cor_m*vol_bsm[:None]
        #bsm_chol_m=np.linalg.cholesky(bsm_cov_m)

        prices=[]
        for i in range(n_assets):
            one_asset = forward[i]*np.exp(-0.5*texp*np.cov(chol_m@znorm_m)[i,i]+np.sqrt(texp)*chol_m[i,:]@znorm_m)
            prices.append(one_asset)
